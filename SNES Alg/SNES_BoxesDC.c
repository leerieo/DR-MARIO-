...
void callGenVirus(){
	...
	for (size_t i = 0, maxGenViruses = Players[PLAYER1].maxGenViruses; i < maxGenViruses; i++) {
		if(Players[PlayerNum].virusLevel > 20){    //Only do this for later levels where its required
			for(int j = 0; j < 15; j++){    //Runs 15 times
				Rand();
			}
		}
		GenVirus();
...
int main(int argc, char** argv) {
	...
	for (int i = 0; i<66535; i++){
		...
		InitNumGenViruses();
		InitGenVirusData();
		virusGoal = Players[PLAYER1].maxGenViruses;

		callGenVirus();
		
		virusesGenerated = Players[PLAYER1].numGenViruses; 

		while(virusesGenerated==virusGoal){
			//printf("virusesGenerated: %d\n",virusesGenerated);  
			//printf("virusGoal: %d\n",virusGoal); 
	
			Players[PLAYER1].virusLevel += 1;
