...
void callGenVirus(int numUpdates){

	for (size_t i = 0, maxGenViruses = Players[PLAYER1].maxGenViruses; i < maxGenViruses; i++) {
		if(Players[PlayerNum].virusLevel > 20){    //Only do this for later levels where its required
...
void resetBottle(){
	...
	for (int y = 0; y < BOTTLE_HEIGHT; y++) {
		for (int x = 0; x < BOTTLE_WIDTH; x++) {
			Players[PLAYER1].bottle[y * MATRIX_WIDTH + x] = CELL_EMPTY;
		}
	}
...
int main(int argc, char** argv) {
    ...
    Players[PLAYER1].virusLevel = MAXVIRUSLEVEL;
	...
	for (int i = 0; i<66535; i++){
        ...
        //InitNumGenViruses
        Players[PLAYER1].maxGenViruses = 0;
        Players[PLAYER1].numGenViruses = 0; 
		
		InitGenVirusData();
		callGenVirus(numUpdates);
		

		while(Players[PLAYER1].maxGenViruses==Players[PLAYER1].numGenViruses){

            //InitNumGenViruses
			Players[PLAYER1].maxGenViruses += 1; 
            Players[PLAYER1].numGenViruses = 0; 

            if (Players[PLAYER1].maxGenViruses == (TOPVIRUSROW*8)){
                break; 
            }else{
                resetBottle();

			    //printf("%d\n",Players[PLAYER1].virusLevel); 
