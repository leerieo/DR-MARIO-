...
class Player:
    def __init__(self):
        ...
        self.nextVirusType = VIRUS_RED #integer, 0 (red), 1(yellow) or 2(blue)
        ...
        self.nextVirusPos = 0
        self.nextVirusPosIndex = 0 #index into all possible positions of a virus
...
#set a cell in the bottle to the appropriate virus color
def SetVirus():
    #yellow
...
#return True if nextVirusPos is valid False otherwise
def ValidVirusPos():
    if Players[PlayerNum].bottle[Players[PlayerNum].nextVirusPos] == CELL_EMPTY:
        print("valid virus in first if")
        ...
        while True:
            ...
            if (beyondEnd or (column == 0) or (column >= BOTTLE_WIDTH - 1) or
                ...
                Temp[0] += 1
                print("valid virus: in if")
                #loop back and check other directions
            else:
                print("valid virus: in else")
                return False
        #if this cell is not empty
    else:
        print("valid virus we are in else")
        return False
...
def GenVirus():
    ...
    Players[PlayerNum].lastVirusPosIndex = 255
    Players[PlayerNum].numVirusGenAttempts = 0
    GenNextVirusPosIndex()
    #rn is loop does not exit
    while True:
...
if __name__ == "__main__":
    ...
    Seed = 0x3400 #take in command line arguments, need to check with all seeds same output?
